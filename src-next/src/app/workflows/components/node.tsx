...
const Port: React.FC<{ port: IOParam; type: "input" | "output" }> = ({
  ...
  >
    {type === "input" && (
      <div className="absolute left-[-4px] -translate-y-1/2 top-1/2 w-2 h-2 border border-background-600 bg-background-800" />
    )}
    {type === "output" && (
      <div className="absolute right-[-4px] -translate-y-1/2 top-1/2 w-2 h-2 border border-background-600 bg-background-800" />
    )}
  </div>
);
